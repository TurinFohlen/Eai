defmodule Eai.Adapter.OpenAI do
  ...
  def to_request_body(messages, model, system_prompt, tools, opts) do
    ...
    # Step 7: OpenAI supports 6 of the 10 sampler fields. The remaining 4
    # (top_k, min_p, repetition_penalty, and our internal :stop_sequences)
    # are NOT supported by the OpenAI Chat Completions API and are silently
    # dropped — the adapter simply does not read them from opts.
    #
    # IMPORTANT wire-format renames: our internal `:stop_sequences` becomes
    # the OpenAI field `stop` (NOT stop_sequences). OpenAI accepts EITHER
    # a string OR an array; we pass the array through (model config
    # convention is an array). If a model config has a single string, that
    # is the user's problem to fix in their config.
    ...
    # Step 7: 6 new fields. Each is conditional — nil means the key is
    # absent from the body. The wire field names match OpenAI's Chat
    # Completions API exactly (top_p, max_tokens, frequency_penalty,
    ...
    body =
      if max_tokens_opt do
        Map.put(body, :max_tokens, max_tokens_opt)
      else
        body
      end

    body =
      if frequency_penalty do
    ...
    body =
      if stop do
        # :stop_sequences (Eai internal) → :stop (OpenAI wire)
  ...
  defp message_to_openai(%{role: :assistant, content: blocks}) do
    ...
    if tool_use_blocks != [] do
      tool_calls =
        Enum.map(tool_use_blocks, fn {:tool_use, kw} ->
          %{
            "id" => kw[:tool_use_id],
            "type" => "function",
            "function" => %{
