defmodule Eai.Adapter.Converse do
  ...
  def to_request_body(messages, model, system_prompt, tools, opts) do
    ...
    # Step 7: Bedrock Converse supports 6 of the 10 sampler fields
    # (temperature was added in Step 6; the new 5 are below). The
    # remaining 4 (min_p, repetition_penalty, frequency_penalty,
    # presence_penalty) are NOT supported by the Converse API and are
    # silently dropped — the adapter simply does not read them from opts.
    #
    # Converse uses camelCase wire names for some fields:
    #   :top_p            → "topP"
    #   :top_k            → "topK"
    ...
    # Step 7: Build the inferenceConfig map conditionally. Each of the 6
    # supported fields is included only when non-nil. The WHOLE object is
    # added to the body only when the map is non-empty. Bedrock's Converse
    # API rejects `null` values inside inferenceConfig, so the per-key
    # omit-when-nil pattern is essential.
    #
    # Wire-format renames (Eai snake_case → Converse camelCase) live here.
    inference_config =
      %{}
      |> maybe_put_inference("temperature", temperature)
      |> maybe_put_inference("topP", top_p)
      |> maybe_put_inference("topK", top_k)
      |> maybe_put_inference("maxTokens", max_tokens_opt)
      |> maybe_put_inference("stopSequences", stop_sequences)
      |> maybe_put_inference("seed", seed)

    body =
      if map_size(inference_config) > 0 do
        Map.put(body, "inferenceConfig", inference_config)
      else
        body
      end
  ...
  # Step 7: per-key omit-when-nil helper for the inferenceConfig map.
  # Returns the map unchanged when the value is nil, matching the
  # omit-when-nil pattern used everywhere else in the chain. Key names
  # are the Converse wire-format names (camelCase) — see the table in
  # docs/step7_changes.md §D.
  defp maybe_put_inference(map, _key, nil), do: map
  defp maybe_put_inference(map, key, value), do: Map.put(map, key, value)
