defmodule Eai.Adapter.Anthropic do
  ...
  def to_request_body(messages, model, system_prompt, tools, opts) do
    ...
    # Step 7: Anthropic supports 4 of the 10 sampler fields. The other 6
    # (min_p, repetition_penalty, frequency_penalty, presence_penalty, seed,
    # and Anthropic's stop_sequences-related helpers) are NOT supported by
    # the Anthropic Messages API and are silently dropped — the adapter
    # simply does not read them from opts. The Direct.run/3 caller has
    # already added them to adapter_opts; we just ignore them.
    ...
    # Step 10: pass-through, no fallback. nil = Anthropic API rejects.
    reasoning_budget_tokens = Keyword.get(opts, :reasoning_budget_tokens)
    ...
    # Step 10: BOTH Anthropic fields with hardcoded fallbacks are now
    # pure pass-through. We do NOT add Eai-side fallbacks for API
    # compatibility. The user said: "我们没必要为api那一边的兼容性
    # 问题,在主代码的这种预设理想情况下改什么". If the caller (or
    # model config) doesn't supply `max_tokens` or `reasoning_budget_tokens`,
    # we emit nil and let the Anthropic API reject the request. This
    # is the explicit design — Eai does NOT shield callers from
    # provider-required fields. Callers / model configs MUST declare
    # both fields if they want a valid request.
    #
    # Pre-Step-10 behaviour for reference: `max_tokens: max_tokens_opt || 8192`
    # (Anthropic) and `budget_tokens: 5000` (Anthropic thinking) were
    # both retained from before Step 7 to avoid breaking existing
    # requests. Step 10 removes those fallbacks.
    body = %{
      model: model,
    ...
    body =
      if effort do
        Map.put(body, :thinking, %{type: "enabled", budget_tokens: reasoning_budget_tokens})
      else
        body
      end
    ...
    # Step 7: top_p, top_k, stop_sequences. Each is conditional — nil
    # means the key is absent from the body. The wire field names match
    # Anthropic's Messages API exactly (top_p, top_k, stop_sequences).
    body =
      if top_p do
    ...
    body =
      if stop_sequences do
        Map.put(body, :stop_sequences, stop_sequences)
      else
        body
      end

    # Step 9: the `anthropic-beta: output-128k-2025-02-19` header (or any
    # other Anthropic beta header) is no longer contributed by the adapter.
    # The decision to opt in is per-model and lives in
    # `config/models/<name>.exs` (field `anthropic_beta: [String.t()]`).
    # `Eai.LLM.Direct.run/3` reads it via `Map.get(opts, :anthropic_beta,
    # entry[:anthropic_beta])`, joins the list with ", " when non-empty,
    # and feeds the result to `build_headers/3` as `extra_headers`. When
    # the list is nil/[], no beta header is sent and the adapter's
    # `headers: []` flows through unchanged. Bedrock-managed Anthropic
    # (Converse adapter) and OpenAI are unaffected — they have their own
    # wire formats and their own `build_headers/3` clauses.
