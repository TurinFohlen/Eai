defmodule Eai.Hub.Pipeline do
  ...
  def pre_hooks(mod, fun, args) do
    ...
    hooks()
    |> Enum.reduce_while({:ok, args}, fn {hook_mod, _prio}, {_status, current_args} ->
  ...
  def post_hooks(mod, fun, args, result) do
    ...
    hooks()
    |> Enum.reduce_while({:ok, result}, fn {hook_mod, _prio}, {:ok, current_result} ->
      current_payload = %{payload | args: args}
